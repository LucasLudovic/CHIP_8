...
int opcode_8XYE(emulator_t *emulator, UNUSED Uint8 b1, UNUSED Uint8 b2, Uint8 b3)
{
    if (emulator == NULL || emulator->cpu == NULL)
        return display_error("Unable to access CPU with opcode 8XYE\n");
    emulator->cpu->memory_register[NB_REGISTER - 1] = (emulator->cpu->memory_register[b3] & 0x80) >> 7;
